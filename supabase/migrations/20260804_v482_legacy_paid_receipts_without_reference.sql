-- Footloose Alley Studio Manager v4.8.2 legacy paid receipt recovery without UPI reference.
...
begin
  ...
  set payment_status = 'Paid',
      amount_paid = case when coalesce(amount_paid, 0) > 0 then amount_paid else coalesce(amount_due, original_amount, 0) end,
  ...
  where id = p_registration_id
    and (
      (payment_status = 'Pending' and payment_reference is not null and length(trim(payment_reference)) >= 4)
      or (payment_status = 'Paid' and receipt_number is null)
    )
  returning * into updated_registration;

  if updated_registration.id is null then
    raise exception 'A referenced pending payment or paid registration without a receipt is required.' using errcode = 'P0002';
  end if;
