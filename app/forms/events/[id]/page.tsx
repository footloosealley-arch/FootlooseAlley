...
const automaticDiscountPercent = (groupSize: number, dateCount: number) =>
  Math.max(dateCount >= 3 ? 20 : dateCount === 2 ? 10 : 0, groupSize >= 5 ? 20 : groupSize >= 3 ? 10 : 0);
...
export default function PublicEventRegistrationPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const automaticPercent = automaticDiscountPercent(groupSize, selectedDates.length);
  const automaticOriginal = eventItem ? eventItem.fee * groupSize * selectedDates.length : 0;
  const automaticSaving = Number((automaticOriginal * automaticPercent / 100).toFixed(2));
  const automaticTotal = Number((automaticOriginal - automaticSaving).toFixed(2));
            ...
            <section className="p-5 sm:p-8">
              ...
              {selectedCapacity === 0 && selectedDates.length > 0 ? <p className="mt-6 rounded-xl bg-amber-50 p-4 font-semibold text-amber-900">This event is fully booked.</p> : (
                <form className="mt-7 space-y-5 border-t border-rose-100 pt-7" onSubmit={register}>
                  ...
                  {selectedDates.length > 0 && <div className="rounded-xl bg-violet-50 p-3 text-sm text-violet-900"><p><strong>Booking total:</strong> {groupSize} × {money.format(eventItem.fee)} × {selectedDates.length} date{selectedDates.length === 1 ? "" : "s"} = {money.format(automaticOriginal)}</p>{automaticPercent > 0 && <p className="mt-2 font-semibold text-emerald-800">Automatic {automaticPercent}% discount applied · You save {money.format(automaticSaving)} · Total {money.format(automaticTotal)}</p>}</div>}
                  {eventItem.fee > 0 && <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-4"><label className={labelClass}>Extra coupon code (optional)</label><p className="mt-1 text-xs text-slate-600">Multi-day and group discounts are applied automatically. A manual coupon only replaces it when the coupon gives a higher discount.</p><div className="mt-2 flex gap-2"><input className="min-h-11 min-w-0 flex-1 rounded-xl border border-rose-200 bg-white px-3 py-2 text-sm uppercase text-slate-900 outline-none focus:border-rose-500 focus:ring-4 focus:ring-rose-100" value={couponCode} onChange={(event) => { setCouponCode(event.target.value.toUpperCase()); setCouponResult(null); }} maxLength={24} placeholder="Enter coupon code" /><Button type="button" variant="outline" className="min-h-11 rounded-xl" disabled={submitting || !couponCode.trim()} onClick={() => void applyCoupon()}>Apply</Button></div>{couponResult && <div className="mt-3 rounded-xl bg-emerald-100 p-3 text-sm text-emerald-900"><p className="font-bold">{couponResult.code} applied — {couponResult.percent}% off</p><p className="mt-1"><span className="line-through">{money.format(eventItem.fee * groupSize * selectedDates.length)}</span> <strong className="ml-2 text-base">{money.format(couponResult.amount)}</strong> · You save {money.format(couponResult.discount)}</p></div>}</div>}
