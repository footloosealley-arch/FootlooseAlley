    ...
    <>
      <Dialog open={open} onOpenChange={(next) => !saving && onOpenChange(next)}>
        <DialogContent className="max-h-[calc(100dvh-1rem)] overflow-y-auto sm:max-w-4xl">
          ...
          {items.some((item) => item.payment_reference && (item.payment_status === "Pending" || (item.payment_status === "Paid" && !item.receipt_number))) && <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4"><h3 className="font-bold text-amber-950">Payments awaiting verification or receipt</h3><p className="mt-1 text-sm text-amber-800">Review the submitted UPI reference before confirming payment or issuing a missing receipt.</p></div>}
            ...
            <div className="grid gap-3 lg:grid-cols-2">
              {filtered.map((item) => (
                <article key={item.id} className="rounded-2xl border p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">{item.payment_reference && (item.payment_status === "Pending" || (item.payment_status === "Paid" && !item.receipt_number)) && <Button type="button" size="sm" onClick={() => void confirmPayment(item)} disabled={saving}><BadgeCheck /> {item.payment_status === "Paid" ? "Issue receipt" : "Confirm payment"}</Button>}{item.payment_status === "Paid" && item.receipt_number && <><Button type="button" size="sm" variant="outline" onClick={() => setReceipt(item)}><ReceiptText /> Receipt</Button><Button size="sm" variant="outline" render={<a href={paymentConfirmationUrl(item)} target="_blank" rel="noreferrer" />}><MessageCircle /> Confirm via WhatsApp</Button></>}<Button size="sm" variant="outline" render={<a href={`https://wa.me/${whatsappPhone(item.phone)}`} target="_blank" rel="noreferrer" />}><MessageCircle /> Chat</Button><Button type="button" size="sm" variant="outline" onClick={() => startEdit(item)}><Pencil /> Edit</Button><Button type="button" size="sm" variant="outline" className="text-destructive hover:text-destructive" onClick={() => setDeleting(item)}><Trash2 /> Delete</Button></div>
                </article>
              ))}
            </div>
          ))}
