...
export default function InstructorManagement() {
  ...
  return <div className="space-y-6">
    <PageHeader title="Instructors" description="Manage your teaching team, specialties, and availability." action={<Button type="button" onClick={() => { setEditing(null); setDialog(true); }}><Plus /> Add instructor</Button>} />
    ...
    <section className="rounded-2xl border bg-card shadow-sm">
      <div className="flex flex-col gap-3 border-b p-4 sm:flex-row"><Input className="sm:max-w-sm" type="search" aria-label="Search instructors" placeholder="Search name, phone, or specialization" value={search} onChange={e => setSearch(e.target.value)} />
        ...
        <Button type="button" className="sm:ml-auto" variant="outline" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? "animate-spin" : ""} /> Refresh</Button></div>
      {loading ? <div className="p-5"><LoadingCard /></div> : error ? <div className="p-5"><ErrorCard message={error} onRetry={() => void load()} /></div> : filtered.length === 0 ? <div className="p-12 text-center text-sm text-muted-foreground">No instructors match your filters.</div> : <div className="divide-y">{filtered.map(item => <article key={item.id} className="flex flex-col gap-4 p-5 lg:flex-row lg:items-center">
        ...
        <div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" render={<a href={`tel:${item.phone}`} aria-label={`Call ${item.name}`} />}><Phone /> Call</Button><Button size="sm" variant="outline" render={<a href={`https://wa.me/${formatWhatsAppPhone(item.phone)}`} target="_blank" rel="noreferrer" aria-label={`WhatsApp ${item.name}`} />}><MessageCircle /> WhatsApp</Button><Button type="button" size="sm" variant="outline" onClick={() => { setEditing(item); setDialog(true); }}><Pencil /> Edit</Button><Button type="button" size="sm" variant={item.status === "Active" ? "destructive" : "default"} disabled={savingId === item.id} onClick={() => void changeStatus(item)}>{item.status === "Active" ? "Deactivate" : "Activate"}</Button></div>
      </article>)}</div>}
    </section><InstructorFormDialog key={editing?.id ?? "new"} open={dialog} instructor={editing} onOpenChange={setDialog} onSaved={() => void load()} />
  </div>;
}
